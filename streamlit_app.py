...
from processing.processing_glaucoma_grading import load_glaucoma_model, predict_glaucoma_severity
...
MODEL_INFO = {
    ...
    "GLAUCOMA": {
        "title": "Glaucoma Severity Grading",
        "module": None,
        "description": (
            "Predicts glaucoma severity from a fundus image using a pretrained EfficientNet model."
        ),
        "recommended_threshold": 0.5,
        "recommended_batch": 8,
        "notes": "Uses your pre-trained EfficientNet model.",
    },
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("DR Grading"):
            set_selected("DRG")
        if st.button("Glaucoma Severity (GLAUCOMA)"):
            set_selected("GLAUCOMA")
    ...
    with main_col:
        ...
        if model_key == "DRG":
            ...
            return
        elif model_key == "GLAUCOMA":
            if uploaded is not None:
                    model = load_glaucoma_model()
                    temp_path = os.path.join("temp_image.jpg")
                    Image.open(uploaded).save(temp_path)
                    result = predict_glaucoma_severity(temp_path, model)
